...
if(empty($_GET['coda'])) {
  header('Location:./');
  exit("Argomento non specificato");
}
...
$sql = "SELECT idt,termine FROM `$ter` WHERE coda=$coda ORDER BY termine";
...
while($riga = $righe->fetch_assoc()) {
  $idt  = $riga['idt'];
  $term = $riga["termine"];
  echo "<a href='#' title='Visualizza la definizione di $term' onclick=\"getDef('$idt')\">".$term."</a>\n";
}
include "dbconfig/dbclose.php";
?>